...
-- LOAD CLIENT IP
-- ==========================
local ipFile = "client_ip.txt"
if not fs.exists(ipFile) then
    error("client_ip.txt not found! Create this file with your desired IP (e.g. 192.168.0.15).")
end
local f = fs.open(ipFile, "r")
local myIP = f.readLine()
f.close()
if not myIP or not myIP:match("^%d+%.%d+%.%d+%.%d+$") then
    error("Invalid IP address format in client_ip.txt.")
end
print("Client IP: "..myIP)
...
print("Sent hello packet to router.")
...
local function requestFile(serverKeyword, filename, password)
    ...
    if not dst then
        print("Unknown server keyword: " .. serverKeyword)
        return
    end
...
local function receiveLoop()
    while true do
        ...
        if type(message) == "table" and message.dst == myIP then
            ...
            if payload.type == "FILE_CHUNK" then
            ...
            elseif payload.type == "FILE_END" then
                -- If this is the first/only chunk, initialize
                if not receivingFile then
                    receivingFile = true
                    fileBuffer = {}
                    expectedChunks = 1
                ...
                if out then
                    ...
                    print("Failed to open file for writing: " .. tostring(fileNameBeingReceived))
                end
                receivingFile = false
                fileBuffer = {}
                expectedChunks = 0
            ...
            parallel.waitForAny(
                ...
                function()
                    local ok, err = pcall(function()
                        shell.run(prog, table.unpack(progArgs))
                    end)
                    if not ok then
                        print("Error running " .. prog .. ": " .. tostring(err))
                    end
                end
            )

        else
            print("Commands: ping <host>, getfile <server> <filename> <password>, list hosts, ip, run <program> [args], exit")
        end
    end
end
